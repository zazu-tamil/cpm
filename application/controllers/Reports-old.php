...
class Reports extends CI_Controller {
    ...
    public function production_report()
	{
       ...
       $sql = "
        ...
        sum(b.produced_qty) as poured_qty,
        ...
        select 
          ...
          $op_sql = " 
            ...
            left join (
                ...
                from qc_inspection_info as a
                left join work_planning_info as b on b.work_planning_id = a.work_planning_id 
                where a.status = 'Active' and b.status != 'Delete' and b.planning_date between '" .$srch_date. "' and '" .$srch_to_date. "' 
                group by b.customer_id, b.pattern_id 
            ) as qb on qb.customer_id = a.customer_id and qb.pattern_id = a.pattern_id
            left join (
                select  
    ...
    public function customer_wise_melting_report()
	{
       ...
       $sql = "
            ...
            left join customer_info as d on d.customer_id = c.customer_id
            left join pattern_info as e on e.pattern_id = c.pattern_id
            where a.`status` = 'Active' and b.status != 'Delete' and c.`status` != 'Delete' and a.melting_date between '" . $srch_from_date . "' and  '". $srch_to_date ."'"; 
       ...
       $sql.= "group by a.melting_heat_log_id order by a.melting_date asc,a.days_heat_no asc, d.company_name, e.pattern_item asc";
        ...
        */
        ...
        $sql = "
            ...
            ((e.bunch_weight * b.pouring_box) - ((if(b.pouring_box > 0 ,(ifnull(g.closed_mould_qty,0)),0)) * e.piece_weight_per_kg)) as liq_metal ,
    ...
    public function customer_wise_rejection_report()
	{
        ...
        */
        ...
        $sql ="
            ...
            left join (select z.work_planning_id , sum(z.produced_qty) as produced_qty from melting_item_info as z where z.`status` != 'Delete' group by z.work_planning_id ) as a1 on a1.work_planning_id = a.work_planning_id  
            ...
            and a.`status` = 'Active' and c.`status` != 'Delete' ";
            if(!empty($srch_customer_id) ){
                $sql.=" and c.customer_id = '". $srch_customer_id ."'"; 
            }
            if(!empty($srch_pattern_id) ){
    ...
    public function work_order_planning_statement()
	{
      ...
      if(isset($_POST['srch_customer_id'])) {
       ...
       if(isset($_POST['srch_pattern_id'])) {
        ...
        $sql = " 
                ...
                LEFT JOIN(
                    ...
                    ON
                        b.work_planning_id = a.work_planning_id
                    WHERE
                        a.status = 'Active' AND b.status != 'Delete' AND b.planning_date BETWEEN IFNULL(
                            (
                            SELECT
                                MAX(z.floor_stock_date)
                            FROM
                ...
                LEFT JOIN(
                    ...
                    ON
                        b.work_planning_id = a.work_planning_id
                    WHERE
                        a.status = 'Active' AND b.status != 'Delete' AND b.planning_date BETWEEN '" .$srch_date. "' AND '" .$srch_to_date. "'
                    GROUP BY
                        b.customer_id,
                        b.pattern_id
                ) AS qb
        ...
        foreach ($query->result_array() as $row)
        {
            if($row['cf_ord_qty'] > 0 || $row['curr_order_qty'] > 0 )
                $data['record_list'][$row['customer']][] = $row;     
        }

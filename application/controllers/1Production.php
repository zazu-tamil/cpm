    ...
    public function work_planning()
    {
        ...
        */
        ...
        $sql ="
            ...
            ((b.qty - ifnull(f.produced_qty,0)) + ((b.qty - ifnull(f.produced_qty,0)) * 10/100)) as wo_req_qty1, 
            round((b.qty - ((ifnull(z1.produced_qty,0) - ifnull(cm.closed_mould_qty,0)) - ifnull(z2.rejection_qty,0))) + ((b.qty - ((ifnull(z1.produced_qty,0) - ifnull(cm.closed_mould_qty,0)) - ifnull(z2.rejection_qty,0))) * 10/100)) as wo_req_qty, 
            round(((b.qty - ifnull(f.produced_qty,0)) + ((b.qty - ifnull(f.produced_qty,0)) * 10 /100))/ d.no_of_cavity) as wo_req_box1 ,
            round(((b.qty - ((ifnull(z1.produced_qty,0) - ifnull(cm.closed_mould_qty,0)) - ifnull(z2.rejection_qty,0))) + ((b.qty - ((ifnull(z1.produced_qty,0) - ifnull(cm.closed_mould_qty,0)) - ifnull(z2.rejection_qty,0))) * 10 /100))/ d.no_of_cavity) as wo_req_box ,
            (round(((b.qty - ifnull(f.produced_qty,0)) + ((b.qty - ifnull(f.produced_qty,0)) * 10 /100))/ d.no_of_cavity) * d.bunch_weight) as wo_box_wt1,
            (round(((b.qty - ((ifnull(z1.produced_qty,0) - ifnull(cm.closed_mould_qty,0)) - ifnull(z2.rejection_qty,0))) + ((b.qty - ((ifnull(z1.produced_qty,0) - ifnull(cm.closed_mould_qty,0)) - ifnull(z2.rejection_qty,0))) * 10 /100))/ d.no_of_cavity) * d.bunch_weight) as wo_box_wt,
            (e.core_produced_qty - (if(d.pattern_type = 'Core',ifnull(f.produced_qty,0),0 ))) as core_avail_qty,
            f.produced_qty,
            d2.pattern_item as child_pattern_1,
            d3.pattern_item as child_pattern_2,
            d4.pattern_item as child_pattern_3,
            d5.pattern_item as child_pattern_4,
            d6.pattern_item as child_pattern_5,
            d7.pattern_item as child_pattern_6,
            d8.pattern_item as child_pattern_7,
            ...
            left join pattern_info as d2 on d2.pattern_id = d.child_pattern_1
            left join pattern_info as d3 on d3.pattern_id = d.child_pattern_2
            left join pattern_info as d4 on d4.pattern_id = d.child_pattern_3
            left join pattern_info as d5 on d5.pattern_id = d.child_pattern_4
            left join pattern_info as d6 on d6.pattern_id = d.child_pattern_5
            left join pattern_info as d7 on d7.pattern_id = d.child_pattern_6
            left join pattern_info as d8 on d8.pattern_id = d.child_pattern_7
            ...
            where a.`status` != 'Delete' and a.`status` != 'Close' and b.status = 'Active' and a.customer_id != '23' and a.customer_id != '20'
            group by a.customer_id, a.work_order_id, b.work_order_item_id
            order by d.match_plate_no,c.company_name asc, b.delivery_date asc 
        ";
        ...
        foreach ($query->result_array() as $row)
        {
            if($row['wo_req_qty'] > 0) {
               ...
               // if($row['child_pattern_1'] != '')  
                {
                    $data['child_record_list'][$row['pattern_item']][] = $row;
                } 
      ...
      if(isset($_POST['srch_date'])) {
       ...
       if(isset($_POST['srch_shift'])) {
        ...
        $sql = "
                    ...
                    (if(ifnull(a.prt_work_plan_id,0) != 0 ,  ifnull(a.no_of_cavity,1) , b.no_of_cavity )) as no_of_cavity,
                    a.prt_work_plan_id,
                    b.child_pattern_1,
                    b.child_pattern_2,
                    b.child_pattern_3,
                    ...
                    (a.planned_box *  (if(ifnull(a.prt_work_plan_id,0) != 0 ,  ifnull(a.no_of_cavity,1) , b.no_of_cavity ))) as planned_qty  
            ...
            foreach($work_plan_id as $plan_id => $box)
            {
                if(($box > 0) or ($leftout_box[$plan_id] > 0)){
                    ...
                    $this->db->query("insert into melting_child_item_info (melting_heat_log_id,work_planning_id,produced_qty) (
                                        ...
                                        a1.work_planning_id, 
                                        (ifnull(a1.no_of_cavity,1) * '". $box ."') as produced_qty
                                        from work_planning_info as a1  
